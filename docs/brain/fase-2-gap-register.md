...
## FECHADO NESTA RODADA (25/09/2026 — revisão independente)
...
| **`comparison` some da resposta em três caminhos de saída antecipada** (achado da revisão independente de 25/09) | aberto, registrado nesta rodada | baixo — em `synthesis.ts`, os retornos `external_processing_forbidden`, `no_provider` e `provider_error` chamam `base({...})` sem passar `comparison`, então mesmo numa pergunta que É comparação (`plano.status === "ready"`), o campo fica `undefined` em vez de `true` nesses três caminhos — só `too_many`, `incomplete` e o caminho feliz o preenchem | `src/modules/brain/synthesis.ts` — as três chamadas `base({...})` sem `comparison:` nos blocos `!externo.allowed`, `!provider` e `catch` do provedor | preencher `comparison: plano.status === "ready" || undefined` nos três `base({...})` que faltam, com teste que prove o selo "Comparação" aparecendo também nesses caminhos | não | não | não |
| **Fallback `?? c.evidenceIndex` em `citacoesParaTela` nunca exercitado** (achado da revisão independente de 25/09) | aberto, registrado nesta rodada | baixo — `posicao.get(aceitas[c.evidenceIndex]!) ?? c.evidenceIndex` devolve o índice ORIGINAL (sobre `aceitas`) se a evidência aceita não for encontrada em `evidencias`; como `aceitas` é sempre um subconjunto de `evidencias` na cadeia real, esse ramo é teoricamente inalcançável — mas se algum dia deixar de ser (ex.: uma evidência aceita filtrada de `evidencias` por engano), o fallback aponta para o card ERRADO em silêncio, sem erro | `src/modules/brain/synthesis.ts`, função `citacoesParaTela` | não corrigir o comportamento (é defensivo); registrar como HARDENING e, se algum teste algum dia forçar esse ramo, trocar o fallback por uma asserção que quebre alto em vez de apontar card errado | não | não | não |
