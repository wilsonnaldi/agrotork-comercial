...
| `13_expiracao.sql` | Expiração automática: só o `sent` vencido vira `expired`; o que vence hoje continua valendo; `draft`, `approved`, `rejected`, `cancelled`, o já `expired`, o sem validade e o descartado ficam intactos; carimbos e totais não são tocados; execuções repetidas não têm efeito; `quotes_list` (base dos relatórios) continua batendo; privilégios, `security definer`, `search_path`, índice, e o vendedor que não executa a função nem alcança orçamento alheio |
| `14_auditoria.sql` | Trilha de auditoria: cadastro e troca de papel auditados com ator sistema; criação e alteração com ator de sessão real; diff só das colunas alteradas; update inócuo que não gera evento; recálculo de totais que não vira `quote.updated` fantasma; verbos de status (`status_changed`, `approved`, `rejected`, `cancelled`) e `quote.expired` com ator sistema; custo e preço com evento próprio; token de link redigido e ausente do log inteiro; visita pública que não gera linha; vendedor que lê zero linhas e não escreve; administrador que lê mas não corrige nem apaga; dono da tabela barrado em UPDATE, DELETE e TRUNCATE; rastro que sobrevive à exclusão do orçamento e à desativação do usuário; privilégios, `security definer`, `search_path`, RLS e txid |
