...
#
# M1  todas as migrations → suite 33 passa (17 asserções, 0 erro)
...
# M4  reaplicar as duas migrations depois da remocao → suite 33 passa de novo
# M5  03-remover-brain (Fase 1) recusa rodar com o Lote A aplicado
# M6  pgvector: nao instalado, nenhuma coluna/indice vetorial
# ============================================================
set -uo pipefail
...
suite33() { q -q -f supabase/db-tests/33_brain_memoria.sql 2>&1; }
...
echo "▶ M1: migrations + suite 33"
montar || nok "M1: montagem"
SAIDA=$(suite33)
N=$(grep -c "NOTICE" <<< "$SAIDA"); E=$(grep -cE "ERROR|FALHOU" <<< "$SAIDA")
if [ "$N" = "17" ] && [ "$E" = "0" ]; then ok "M1: suite 33 — 17 asserções, 0 erro"; else nok "M1: asserções=$N erros=$E"; grep -E "ERROR|FALHOU" <<< "$SAIDA" | head -3; fi
...
for s in 25_brain 27_brain_exclusoes 28_brain_pontes 29_brain_volatilidade 30_brain_reconciliacao 31_brain_fidelidade 32_brain_policies; do
  ...
  EF1=$((EF1 + $(grep -cE "ERROR|FALHOU" <<< "$S"))); NF1=$((NF1 + $(grep -c "NOTICE" <<< "$S")))
done
if [ "$TAB" = "9" ] && [ "$PONTES" = "3" ] && [ "$LIGADAS" = "0" ] && [ "$EF1" = "0" ]; then
  ok "M2: 9 tabelas da Fase 1, 3 pontes desligadas, suites 25–32 verdes ($NF1 asserções) ao lado do Lote A"
else nok "M2: tab=$TAB pontes=$PONTES ligadas=$LIGADAS erros_fase1=$EF1"; fi
...
SAIDA=$(suite33)
N=$(grep -c "NOTICE" <<< "$SAIDA"); E=$(grep -cE "ERROR|FALHOU" <<< "$SAIDA")
if [ "$N" = "17" ] && [ "$E" = "0" ]; then ok "M4: reaplicado, suite 33 passa de novo (17/0)"; else nok "M4: asserções=$N erros=$E"; fi
...
if [ "$VEC" = "0" ]; then ok "M6: pgvector nao instalado, zero colunas e indices vetoriais"; else nok "M6: vec=$VEC"; fi
