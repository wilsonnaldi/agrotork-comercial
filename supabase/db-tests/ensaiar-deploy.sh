...
#
# D1  aplicar: as três migrations + conferências + registro, num COMMIT
...
# D7  religar a ponte e repor os eventos do período desligado
# ============================================================
set -uo pipefail
...
montar_pre_brain() {
  ...
  q -q -c "drop table if exists supabase_migrations.schema_migrations_antes_20260911" >/dev/null
...
REG=$(q -c "select count(*) from supabase_migrations.schema_migrations where version in ('20260911130000','20260911140000','20260911150000')")
SUJEIRA=$(q -c "select (select count(*) from brain.leads) + (select count(*) from public.quotes) + (select count(*) from brain.events where processing::text <> 'skipped')")
if [ "$TAB" = "9" ] && [ "$REG" = "3" ] && [ "$SUJEIRA" = "0" ]; then
  ok "D1: 9 tabelas, 3 versões registradas, 0 linha de fumaça viva"
  echo "$SAIDA" | grep -q "Fumaca OK" && ok "D1: a fumaça exercitou lead + orçamento + evento" || nok "D1: a fumaça não rodou"
else
...
cp supabase/operacao/02-aplicar-brain.sql /tmp/02-sabotado.sql
python3 - <<'PY'
p='/tmp/02-sabotado.sql'
s=open(p).read()
s=s.replace("-- ── Pós-condições estruturais ───────────────────────────────",
            "alter view brain.journey_entries set (security_invoker = false);\n-- ── Pós-condições estruturais ───────────────────────────────")
open(p,'w').write(s)
PY
...
if echo "$SAIDA" | grep -q "O BRAIN JA TEM DADO" && [ "$EXISTE" = "1" ] && [ "$LEADS" = "1" ]; then
...
LEADS=$(q -c "select count(*) from brain.leads")
REG=$(q -c "select count(*) from supabase_migrations.schema_migrations where version like '202609111%'")
if [ "$EXISTE" = "1" ] && [ "$TRIG" = "0" ] && [ "$COPIA" = "1" ] && [ "$LEADS" = "1" ] && [ "$REG" = "3" ]; then
...
montar_pre_brain
sed 's/  if v_eventos <> 1 then/  v_eventos := v_eventos + 99;\n  if v_eventos <> 1 then/' \
...
if echo "$SAIDA" | grep -q "publicou 100 evento" && [ "$EXISTE" = "0" ] && [ "$REG" = "0" ] && [ "$QUOTES" = "0" ]; then
...
[ "$FALHAS" = "0" ] && echo "✔ deploy e reversão ensaiados nos 7 cenários" || { echo "✗ $FALHAS falha(s)"; exit 1; }
