...
if [ "$TAB" = "9" ] && [ "$REG" = "9" ] && [ "$SUJEIRA" = "0" ]; then
  ...
  grep -q "SEM nenhum evento novo no BRAIN" <<< "$SAIDA" && ok "D1: a venda passou sem tocar no BRAIN (pontes desligadas)" || nok "D1: a fumaça não provou o desacoplamento"
  grep -q "venda ganha, lead convertido, pedido ligado" <<< "$SAIDA" && ok "D1: a reconciliação reproduziu o fato" || nok "D1: a reconciliação não reproduziu o fato"
  grep -q "divergencias_erp() vazio" <<< "$SAIDA" && ok "D1: relatório de divergências vazio" || nok "D1: sobrou divergência"
  grep -q "Dado comercial intacto" <<< "$SAIDA" && ok "D1: as 9 tabelas de negócio com o mesmo retrato de antes" || nok "D1: dado comercial alterado"
  ...
  grep -q "Fumaca desfeita" <<< "$SAIDA" && ok "D1: a fumaça se desfez por inteiro" || nok "D1: a fumaça não se desfez"
else
...
REG=$(q -c "select count(*) from supabase_migrations.schema_migrations where version like '20260911%'")
if grep -q "journey_entries esta com reloptions" <<< "$SAIDA" && [ "$EXISTE" = "0" ] && [ "$REG" = "0" ]; then
...
SAIDA=$(q -f supabase/operacao/02-aplicar-brain.sql 2>&1)
EXISTE=$(q -c "select count(*) from pg_namespace where nspname='brain'")
if grep -q "Alguem a mudou depois" <<< "$SAIDA" && [ "$EXISTE" = "0" ]; then
...
if grep -q "O BRAIN TEM CONTEUDO" <<< "$SAIDA" && [ "$EXISTE" = "1" ] && [ "$LEADS" = "1" ]; then
...
if grep -q "Evento com total" <<< "$SAIDA" && [ "$EXISTE" = "0" ] && [ "$REG" = "0" ] && [ "$QUOTES" = "0" ]; then
...
SAIDA=$(q -f supabase/operacao/03-remover-brain-sem-dados.sql 2>&1)
EXISTE=$(q -c "select count(*) from pg_namespace where nspname='brain'")
if [ "$EVENTOS" -ge 1 ] && grep -q "EVENTO(S)" <<< "$SAIDA" && [ "$EXISTE" = "1" ]; then
...
for CASO in view funcao fk; do
  ...
  SAIDA=$(q -f supabase/operacao/03-remover-brain-sem-dados.sql 2>&1)
  EXISTE=$(q -c "select count(*) from pg_namespace where nspname='brain'")
  if grep -qE "Dependente fora do brain|CITA brain" <<< "$SAIDA" && [ "$EXISTE" = "1" ]; then
...
EXISTE=$(q -c "select count(*) from pg_namespace where nspname='brain'")
REG=$(q -c "select count(*) from supabase_migrations.schema_migrations where version like '20260911%'")
if grep -q "MODO DESACOPLADO violado" <<< "$SAIDA" && [ "$EXISTE" = "0" ] && [ "$REG" = "0" ]; then
...
if [ "$JOBS" = "1" ] && [ "$HORA" = "* * * * *" ] && [ "$ATIVO" = "t" ] && [ "$JOBS2" = "1" ] \
   && grep -q "MODO DESACOPLADO violado" <<< "$RECUSA"; then
