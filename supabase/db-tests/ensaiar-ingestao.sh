...
#     ficam, 030000 e 040000 saem; pgvector ausente
...
suites() { for s in 33_brain_memoria 34_brain_memoria_hardening 35_brain_ingestao 36_brain_busca_calibracao; do q -q -f "supabase/db-tests/$s.sql" 2>&1; done; }
...
q -q -v ON_ERROR_STOP=1 -f supabase/migrations/20260912040000_brain_busca_calibracao_piloto.sql >/dev/null 2>&1 || nok "I7: aplicar 040000"
q -q -c "insert into supabase_migrations.schema_migrations (version, name) values ('20260912030000','brain_ingestao'), ('20260912040000','brain_busca_calibracao_piloto') on conflict do nothing" >/dev/null
RB=$(retrato)
SAIDA=$(for s in 35_brain_ingestao 36_brain_busca_calibracao; do q -q -f "supabase/db-tests/$s.sql" 2>&1; done); N35=$(grep -c "NOTICE" <<< "$SAIDA"); E35=$(grep -cE "ERROR|FALHOU" <<< "$SAIDA")
...
q -q -v ON_ERROR_STOP=1 -f supabase/migrations/20260912040000_brain_busca_calibracao_piloto.sql >/dev/null 2>&1
q -q -c "insert into supabase_migrations.schema_migrations (version, name) values ('20260912030000','brain_ingestao'), ('20260912040000','brain_busca_calibracao_piloto') on conflict do nothing" >/dev/null
...
q -q -c "do \$\$ declare v uuid; i uuid; begin select id into v from brain.document_versions where file_sha256 = repeat('dd', 32);
   i := brain.ingestion_start(v, 'pdf_text', 'x', 'lote-b.2', 'ensaio', false, 2);
...
SAIDA=$(q -f supabase/operacao/08-remover-lote-b-sem-dados.sql 2>&1)
AINDA=$(q -c "select (to_regclass('brain.knowledge_queries') is not null)::int + (select count(*) from supabase_migrations.schema_migrations where version in ('20260912030000','20260912040000'))")
if grep -q "conteudo repetido em paginas diferentes" <<< "$SAIDA" && [ "$AINDA" = "3" ]; then ok "I8: 08 parou (conteudo repetido entre paginas) e nada foi removido"; else nok "I8: ainda=$AINDA"; tail -3 <<< "$SAIDA"; fi
